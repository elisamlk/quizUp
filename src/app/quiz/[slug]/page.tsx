...
export default async function QuizPage({
  ...
  // Même catégorie (principal maillage)
  const sameCategory = all.filter(
    (q) => q.slug !== quiz.slug && q.category.slug === quiz.category.slug,
  );

  // "Quiz similaires" = on prend une sélection stable
  const related = sameCategory.slice(0, 8);

  // Next quiz: on prend le 1er de cette sélection
  const nextQuiz = related[0]
    ? { slug: related[0].slug, title: related[0].title }
    : null;

  // Cross-category populaire (découverte + pages vues)
  const popularCross = all
    .filter((q) => q.slug !== quiz.slug && q.isPopular)
    .slice(0, 8);
  ...
  const estMinutes = Math.max(3, Math.round(quiz.questions.length * 0.35));
  ...
  return (
    <main className="page">
      ...
      <header className="quizHero">
        {quiz.images?.cover ? (
          <div
            ...
            style={{ backgroundImage: `url("${quiz.images.cover}")` }}
            aria-hidden="true"
          />
        ) : null}
      ...
      <QuizDisplay related={related} categorySlug={quiz.category.slug} categoryName={quiz.category.name}>
  <QuizPlayer key={quiz.slug} quiz={quiz} nextQuiz={nextQuiz} />
</QuizDisplay>
        </section>

        {/* Texte SEO supplémentaire */}
        <section className="quizSeoText">
          <h2 className="sectionTitle">Ce que tu vas trouver dans ce quiz</h2>
          <ul className="seoList">
            <li>{quiz.questions.length} questions au format QCM</li>
            <li>Correction immédiate et score final</li>
            <li>Explications pour progresser (quand elles sont disponibles)</li>
          </ul>

          <p className="seoP">
            Astuce : refais le quiz pour battre ton record et compare ton score
            avec tes amis en partageant ta carte de résultat.
          </p>
        </section>
        {/* Populaires (cross-category) : augmente les pages vues */}
