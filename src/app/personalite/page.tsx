...
export async function generateMetadata({
  ...
  const canonical = p > 1 ? `/personalite?p=${p}` : "/personalite";

  let resultsCount: number | null = null;

  if (hasFilters) {
    const tests = getAllPersonalityTests();

    resultsCount = tests.filter((test) => {
      const okCat = !selectedCat || test.category.slug === selectedCat;
...
function buildPersonalityUrl({
  ...
  return qs ? `/personalite?${qs}` : "/personalite";
}
...
export default async function PersonalityIndexPage({
  ...
  const tests = getAllPersonalityTests();
  const categories = getAllPersonalityCategories();
...
<section className="heroLandingSection personalityHeroSection">
  <div className="heroLandingContent">
    ...
    <p className="heroLandingSub">
      Découvre des tests de personnalité autour du cinéma, de la culture,
      des goûts et des styles de vie. Réponds aux questions, découvre ton
      profil dominant et partage ton résultat.
    </p>

    <p className="pageSubtitle">
      {total} test{total > 1 ? "s" : ""} • Page {currentPage} / {totalPages}
    </p>
      ...
      <div className="quizListSection">
        <section className="filters">
          <form className="searchBar" action="/personalite">
            {selectedCat ? (
          ...
          <div className="row">
            <div className="rowTrack rowTrack--categories">
              <Link
                href={buildPersonalityUrl({ q: qRaw, p: 1 })}
        ...
        <section className="quizList">
          {pageItems.map((test, idx) => {
            const img =
              test.images?.thumbnail || "/images/placeholder-cover.jpg";
          ...
          {total === 0 ? (
            <p className="emptyState">
              Aucun test de personnalité ne correspond à ta recherche.
            </p>
          ) : null}
        </section>
          ...
          <nav className="pager" aria-label="Pagination">
            ...
            <div className="pagerNums" aria-label="Pages">
              {getPageItems(currentPage, totalPages).map((item, idx) => {
                ...
                const page = item;
                const href = buildPersonalityUrl({
                  cat: selectedCat || undefined,
                  q: qRaw || undefined,
                  p: page,
                });
