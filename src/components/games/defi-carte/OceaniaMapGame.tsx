...
import {
  OceaniaMap,
} from "@/app/jeux/defi-carte/OceaniaMap";
...
export function OceaniaMapGame() {
  const [
    isStarted,
    setIsStarted,
  ] = useState(false);
  ...
  const foundCountries =
    useMemo(() => {
      return OCEANIA_COUNTRIES.filter(
        (country) =>
          foundCodes.includes(
            country.code
          )
  ...
  function getShareText() {
    return `J'ai trouvé ${foundCodes.length} pays sur ${OCEANIA_COUNTRIES.length} au Défi Carte Océanie sur QuizUp ! 🌏`;
  }
  ...
  async function shareResult() {
    ...
    try {
      ...
      console.error(
        "Erreur lors du partage du Défi Carte Océanie :",
        error
      );
    } finally {
      setIsSharing(
            ...
            {!isStarted ? (
              ...
              <div className="mapGame__inputContainer">
                <input
                  ...
                  aria-label="Nom d'un pays d'Océanie"
                />
              </div>
            )}
          </div>
