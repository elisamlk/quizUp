...
import {
  NorthAmericaMap,
} from "@/app/jeux/defi-carte/NorthAmericaMap";
...
export function NorthAmericaMapGame() {
  const [
    isStarted,
    setIsStarted,
  ] = useState(false);
  ...
  const normalizedCountries =
    useMemo(() => {
      return NORTH_AMERICA_COUNTRIES.map(
        (country) => ({
          ...country,
  ...
  const foundCountries =
    useMemo(() => {
      return NORTH_AMERICA_COUNTRIES.filter(
        (country) =>
          foundCodes.includes(
            country.code
          )
  ...
  function getShareText() {
    return `J'ai trouvé ${foundCodes.length} pays sur ${NORTH_AMERICA_COUNTRIES.length} au Défi Carte Amérique du Nord sur QuizUp ! 🌎`;
  }
  ...
  async function shareResult() {
    ...
    try {
      ...
      const filename =
        "score-defi-carte-amerique-du-nord.png";
      ...
      if (
        ...
        await navigator.share({
          title:
            "Mon score : Défi Carte Amérique du Nord",
      ...
      console.error(
        "Erreur lors du partage du Défi Carte Amérique du Nord :",
        error
      );
    } finally {
      setIsSharing(
            ...
            {!isStarted ? (
              ...
              <div className="mapGame__inputContainer">
                <input
                  ...
                  aria-label="Nom d'un pays d'Amérique du Nord"
                />
              </div>
            )}
          </div>
