...
/**
 * Share card story: 1080×1920 (Instagram story / TikTok)
 */
async function generateShareImage({
  ...
  ctx.fillText(scoreText, cardX + (cardW - scoreW) / 2, scoreY);
  ...
  ctx.fillText(pctText, cardX + (cardW - pctW) / 2, scoreY + 90);

  // Challenge line
  ctx.font = "700 42px system-ui, -apple-system, Segoe UI, Roboto, Arial";
  ctx.fillStyle = "rgba(255,255,255,0.85)";
  const challenge = "Peux-tu faire mieux ?";
  const chW = ctx.measureText(challenge).width;
  ctx.fillText(challenge, cardX + (cardW - chW) / 2, cardY + cardH - 130);
  ...
  ctx.fillText("Fais le quiz et partage ton score →", 90, H - 74);
...
export function QuizPlayer({
  ...
  function finalizeBest() {
    setBest((prev) => {
      const currentBest = prev ?? readBestScore(quiz.slug);
      if (currentBest === null || score > currentBest) {
        writeBestScore(quiz.slug, score);
        setIsNewBest(true);
        return score;
      }
      setIsNewBest(false);
  ...
  if (step >= total) {
    ...
    const shareText = encodeURIComponent(
      `J’ai fait ${score}/${total} (${scorePct}%) au quiz "${quiz.title}" !`,
    );
  ...
  <div className="resultTop">
    ...
    >
      <div className="resultScoreInner">
        ...
        <div className="resultScorePercent">{scorePct}%</div>
      </div>
    </div>
  </div>
  ...
  {best !== null ? (
    <div className="bestLine">
      {isNewBest ? (
        <span className="bestBadge">Nouveau record</span>
      ) : (
        <span className="bestBadge bestBadge--soft">Meilleur score</span>
      )}
      ...
      <div className="quizPanel">
        <div className="quizTop">
          ...
          <span className="quizScore">
            Score : {score}
            {best !== null ? (
              <span style={{ opacity: 0.75 }}> • Record {best}</span>
            ) : null}
          </span>
        </div>
        ...
        <ul className="quizAnswers">
          {q.answers.map((a, idx) => {
            ...
            if (selected !== null) {
              ...
              else if (chosen && !correct) cls += " isWrong";
              else if (correct) cls += " isCorrectSoft";
              else cls += " isDisabled";
            }
